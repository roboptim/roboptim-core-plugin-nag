...
namespace roboptim
{
  namespace detail
  {
    static void nagSolverCallbackDifferentiable (double xc, double* fc,
                                                 double* gc, Nag_Comm* comm)
    {
      ...
      const Eigen::Map<const function_t::argument_t> x_ (&xc, 1);
  ...
  void NagSolverDifferentiable::solve ()
  {
    // e1 and e2
    ...
    nag_opt_one_var_deriv (detail::nagSolverCallbackDifferentiable, e1_, e2_,
                           &a_[0], &b_[0], max_fun, &x_[0], &f_[0], &g_[0],
                           &comm, &fail);
